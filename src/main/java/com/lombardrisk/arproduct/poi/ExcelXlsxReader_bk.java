...
public abstract class ExcelXlsxReader_bk extends DefaultHandler{
		...
		if (isTElement) {
			...
			if ("row".equals(name)) {
				...
				if (flag){ //该行不为空行且该行不是第一行，则发送（第一行为列名，需要）
					getRows(filePath, sheetName, sheetIndex, curRow, rowlist);
					totalRows++;
				}
				rowlist.clear();
				curRow++;
 ...
 */
    ...
    public static void main(String[] args) throws Exception {  
    	...
    	ExcelXlsxReader reader = new ExcelXlsxReader() {  
    			public void getRows(String filePath,String sheetName,int sheetIndex, int curRow, List<String> rowList) {  
